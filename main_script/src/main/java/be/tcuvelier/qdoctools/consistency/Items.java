...
public class Items {
    public static ItemsResult checkItems(CheckRequest request) throws SaxonApiException {
        ...
        ir.addComparison("Types",
                request.xpathToSet("//(db:enumsynopsis/db:enumname union db:typedefsynopsis[not" +
                        "(preceding-sibling::*[1][self::db:enumsynopsis])]/db:typedefname)/text()"),
                SetHelpers.union(
                        request.htmlToSet("Public Types", "h2", "public-types", true),
                        request.htmlToSet("Related Non-Members", "h2", "related-non-members",
                                false, s -> s.equals("typedef"))
                )
        );
        ...
        ir.addComparison("Public functions",
                request.xpathToSet("//(db:methodsynopsis[not(db:modifier[text() = 'signal'])] " +
                        "union db:constructorsynopsis union db:destructorsynopsis)" +
                        "/db:methodname/text()"), // Methods, constructors, destructors.
                SetHelpers.union(
                        request.htmlToSet("Public Functions", "h2", "public-functions"),
                        request.htmlToSet("Public Slots", "h2", "public-slots"),
                        request.htmlToSet("Reimplemented Public Functions", "h2", "reimplemented" +
                                "-public-functions"), // Example: http://doc.qt.io/qt-5/q3dcamera.html
                        request.htmlToSet("Reimplemented Protected Functions", "h2",
                                "reimplemented-protected-functions"), // Example: http://doc.qt.io/qt-5/qabstractanimation.html#event
                        request.htmlToSet("Static Public Members", "h2", "static-public-members")
                        ...
                        request.htmlToSet("Related Non-Members", "h2", "related-non-members",
                                false, s -> !s.equals("typedef")) // Example: http://doc.qt.io/qt-5/qopengldebugmessage.html
                        // http://doc.qt.io/qt-5/qxmlstreamnotationdeclaration.html
                )
        );
        ir.addComparison("Signals",
                request.xpathToSet("//db:methodsynopsis[db:modifier[text() = " +
                        "'signal']]/db:methodname/text()"),
