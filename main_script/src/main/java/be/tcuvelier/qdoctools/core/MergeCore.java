...
public class MergeCore {
    public static void call(String original, String altered, String merged, MergeType type,
            GlobalConfiguration config) throws SaxonApiException, MalformedURLException,
        ...
        if (FileHelpers.isDocBook(original)) {
            throw new RuntimeException("File format of the original file " + original + " not " +
                    "recognised for input file!");
        }
        if (FileHelpers.isDocBook(altered)) {
            throw new RuntimeException("File format of the altered file " + altered + " not " +
                    "recognised for input file!");
        }
        if (merged != null && FileHelpers.isDocBook(merged)) {
            throw new RuntimeException("File format of the merged file " + merged + " not " +
                    "recognised for input file!");
        }
